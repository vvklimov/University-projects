...
void Queue_Init( Queue *queue ) {
	if (queue != NULL)
	{
		for (int i = 0; i < QUEUE_SIZE; i++) //zaplnime array '*' symbolem
			queue->array[i] = '*';
		queue->firstIndex = 0; 
		queue->freeIndex = 0;
	}
...
int Queue_IsFull( const Queue *queue ) {
	return (queue->firstIndex == nextIndex(queue->freeIndex)); // dalsi prvek po freeIndex je firstIndex - fronta je plna, vraci TRUE 
}
...
void Queue_Front( const Queue *queue, char *dataPtr ) {
	if (!Queue_IsEmpty(queue)) // pokud neni prazdna 
		*dataPtr = queue->array[queue->firstIndex]; // vraci hodnotu 1 elementu
	else
		Queue_Error(QERR_FRONT); //prazdna - chyba
}
...
void Queue_Dequeue( Queue *queue, char *dataPtr ) {
	if (!Queue_IsEmpty(queue)) //pokud neni prazdna
	{
		Queue_Front(queue, dataPtr); //assign hodnoty
		Queue_Remove(queue);	//odstraneni znaku
	}
	else
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	{
		queue->array[queue->freeIndex] = data;	//assign data na freeIndex
		queue->freeIndex = nextIndex(queue->freeIndex); //posun pointeru o 1 
	}
